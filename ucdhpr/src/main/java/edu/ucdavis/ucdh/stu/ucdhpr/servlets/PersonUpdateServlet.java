 ...
 *
 * Note: incoming ppsId is actually ucPathId and incoming oldPpsId is actually the legacy ppsId
 */
public class PersonUpdateServlet extends BaseServlet {
	...
	public void init() throws ServletException {
		...
		dataSource = (DataSource) WebApplicationContextUtils.getRequiredWebApplicationContext(config.getServletContext()).getBean("masterDataSource");
	}
...
log.info(key + ": " +  req.getParameter(key));
	...
	private void processRequest(HttpServletRequest req, String action, JSONObject details) {
		...
		if ("delete".equals(action) || validateRequest(req, details)) {
			...
			try {
				...
				if ("UCDH".equalsIgnoreCase(req.getParameter("ucPathInstitution")) && StringUtils.isNotEmpty(req.getParameter("ppsId")) && StringUtils.isNotEmpty(req.getParameter("ucPathEnd"))) {
					ps.setString(15, "Y");
				} else {
					ps.setString(15, "N");
				}
				if ("UCD".equalsIgnoreCase(req.getParameter("ucPathInstitution")) && StringUtils.isNotEmpty(req.getParameter("ppsId")) && StringUtils.isNotEmpty(req.getParameter("ucPathEnd"))) {
					ps.setString(16, "Y");
				} else {
					ps.setString(16, "N");
				}
				...
				if ("UCD".equalsIgnoreCase(req.getParameter("ucPathInstitution")) && StringUtils.isNotEmpty(req.getParameter("oldPpsId"))) {
					ps.setString(31, nullify(req.getParameter("oldPpsId")));
				...
				if ("UCDH".equalsIgnoreCase(req.getParameter("ucPathInstitution")) && StringUtils.isNotEmpty(req.getParameter("oldPpsId"))) {
					ps.setString(46, nullify(req.getParameter("oldPpsId")));
				...
				ps.setString(54, nullify(req.getParameter("ppsId")));
	...
	/**
	 * <p>Validates the incoming request.</p>
	 *
	 * @param req the <code>HttpServletRequest</code> object
	 * @param details the <code>JSONObject</code> object containing the details of the request
	 */
	...
	/**
	 * <p>Turns an empty string into null.</p>
	 *
	 * @param string the string to nullify
	 * @return the nullified string
	 */
